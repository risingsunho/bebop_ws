  ...
  a_scale_(2.0)
{
  ...
  takeoff_pub1 = nh_.advertise<std_msgs::Empty>("/bebop1/takeoff", 1);
  land_pub1 = nh_.advertise<std_msgs::Empty>("/bebop1/land", 1);
  takeoff_pub2 = nh_.advertise<std_msgs::Empty>("/bebop2/takeoff", 1);
  land_pub2 = nh_.advertise<std_msgs::Empty>("/bebop2/land", 1);
...
//    printf("value: 0x%02X\n", c); //키코드확인
    ...
    switch(c)
    {
      ...
      case KEYCODE_T:
        ...
        takeoff_pub1.publish(val);  
        takeoff_pub2.publish(val);  
        break;
      case KEYCODE_G:
        ROS_DEBUG("land");
		land_pub.publish(val);  
        land_pub1.publish(val);  
        land_pub2.publish(val);  
        break;
