...
sensor_msgs::NavSatFix bebop2GPS;
...
void Flocking()
{
	...
	bebop1plot=GPStoRotatedPlot(myGPS);
	bebop2plot=GPStoRotatedPlot(bebop2GPS);
...
void Move()
{	
	CurrentPosition=GPStoRotatedPlot(myGPS); 
	DesiredPosition=GPStoRotatedPlot(DesiredGPS);   
	DirectionVector=Subvector(DesiredPosition,CurrentPosition);	
	pub_dirv.publish(DirectionVector);
}

void cmd_mode(std_msgs::Int64 m) //Set the DesiredGPS value along with mode value
{    	
	if(m.data==1)  //Making Flocking Plot
	{
		Flocking();
	}
	else if(m.data==2)  //Making in a row
	{
		DesiredGPS=bebopGPS;
		Move();
	}
	else if(m.data==3)  //Following Agent's Command
	{
		DesiredGPS=AgentGPS;
		Move();
	}
	else if(m.data==4) //Take off
	{
	}
	else if(m.data==5) //Land
	{
	}		
}
