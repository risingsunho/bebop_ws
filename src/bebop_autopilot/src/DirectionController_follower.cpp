...
//    ROS_INFO("bebop1 stateyaw : %f",stateYaw.data);
//    ROS_INFO("bebop1 setpointyaw :%f",setpointYaw.data);
...
int main(int argc, char **argv)
{
	...
	ros::init(argc, argv, "DirectionController_follower");    
    ros::NodeHandle nh;
    
    ros::Subscriber sub = nh.subscribe("/bebop1/odom", 1, velCallback);
    ros::Subscriber sub_direction = nh.subscribe("/dirvector1", 1, velCallbackDirection);
    pub_setpoint=nh.advertise <std_msgs::Float64>("/error1", 1);   
