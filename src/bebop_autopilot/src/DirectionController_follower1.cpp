...
//    ROS_INFO("bebop2 stateyaw : %f",stateYaw.data);
//    ROS_INFO("bebop2 setpointyaw :%f",setpointYaw.data);
   	if(stateYaw.data<0)
   	{
...
int main(int argc, char **argv)
{
	...
	ros::init(argc, argv, "DirectionController_follower1");    
    ros::NodeHandle nh_;
    
    ros::Subscriber sub = nh_.subscribe("/bebop2/odom", 1, velCallback);
    ros::Subscriber sub_direction = nh_.subscribe("/dirvector2", 1, velCallbackDirection);
    pub_setpoint=nh_.advertise<std_msgs::Float64>("/error2", 1);
